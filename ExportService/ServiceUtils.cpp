...
TCHAR	g_szServiceName[MAX_PATH] = _T("ExportService");
TCHAR	g_szServiceDescription[MAX_PATH] = _T("PDFhub File Export Service");
...
BOOL InstallService()
{
	...
	keyClasses.Create(HKEY_LOCAL_MACHINE, _T("Software\\InfraLogic\\ExportService"));
	...
	keyClasses.SetKeyValue(_T("Parameters"), _T("c:\\ExportService\\Config.ini"), _T("ConfigFile"));
...
void WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
	...
	strcpy(szEventStr, "ExportService");
	...
	if (g_prefs.LoadPreferencesFromRegistry() == FALSE)
		g_util.Logprintf("CANNOT READ Registry: Software\\InfraLogic\\ExportService\\Parameters..\r\n");
	else {
		g_util.Logprintf("INFO: Registry read: Software\\InfraLogic\\ExportService\\Parameters..\r\n");
	...
	if (dbmaint.InitDB(g_prefs.m_DBserver, g_prefs.m_Database, g_prefs.m_DBuser, g_prefs.m_DBpassword, g_prefs.m_IntegratedSecurity, sErrorMessage) == FALSE) {
		...
		if (dbmaint.LoadAllPrefs(sErrorMessage) == FALSE)
			LogEvent(EVENTLOG_SUCCESS, IDS_ERRORCONNDB, szEventStr, NULL, NULL);

	}
	CString sIniFile =  _T("");
	CString sIniFile2 = _T("");
	CString sIniFile3 = _T("");
	if (dbmaint.LoadConfigIniFile(g_prefs.m_instancenumber, sIniFile, sIniFile2, sIniFile3, sErrorMessage)) {
	...
	g_util.LogprintfTransmit("INFO: Ini config file %s read", sIniFile);
