...
void CEmailClient::SetSMTPStartTLSmode(BOOL useTLS)
{
	m_SMTPuseStartTLS = useTLS;
}
	...
	CString sMailCC, CString sMailSubject, CString sMailBody, BOOL bHtmlBody, CString sAttachmentFile)
{
	...
	if (util.FileExist(sAttachmentFile) == FALSE) {
		util.Logprintf(_T("SMTP ERROR: Attachment file %s not found "), sAttachmentFile);
	}
	if (sSMTPServer == _T("") || sMailFrom == _T("") || sMailTo == _T(""))	{ 
		util.Logprintf(_T("SMTP ERROR: Missing parameters SMTPServer=%s, MailFrom=%s, MailTo=%s"), sSMTPServer, sMailFrom, sMailTo);
		return FALSE;
	}
	bool success = mailman.UnlockComponent(_T("NFRLGC.CB1112020_vZdFLucd1RAZ"));

	if (success != true) {			
		util.Logprintf(_T("SMTP ERROR: Error unlocking SMTP component - %s"), sSMTPServer, mailman.lastErrorText());
		return FALSE;
	}
